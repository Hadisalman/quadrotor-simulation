...
void chatterCallback(const std_msgs::Float32MultiArray::ConstPtr& msg)
{
  // ROS_INFO("I heard: [%f]", msg->data(0));
   geometry_msgs::Pose tmp_wp,tmp_wp2;
	...
	tmp_wp.position.x = msg->data[4]/4;
	tmp_wp.position.y = msg->data[5]/4;
	// cout<<tmp_wp.position.y<<endl;
	tmp_wp.position.z = 23;	//First waypoint is at [0, 0, 5]
...
int main(int argc, char **argv) {
	...
	PoseActionClient poc(nh, "/uav3/action/pose"),poc2(nh2, "/uav3/action/pose");
	poc.waitForServer();
	// poc2.waitForServer();
	ROS_INFO("Pose client initialised.");
	
	// Initialise the TakeoffClient
	TakeoffClient toc(nh, "/uav3/action/takeoff");
	toc.waitForServer();
	...
	LandingClient lnc(nh, "/uav3/action/landing");
	lnc.waitForServer();
